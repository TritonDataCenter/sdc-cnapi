...
CNAPI.prototype.start = function () {
    ...
    async.waterfall([
        ...
        function (callback) {
            self.createCnapiModel(modelOptions, callback);
        },
        function (callback) {
...
CNAPI.prototype.getModel = function (callback) {
    return this.model;
};

CNAPI.prototype.getLog = function (callback) {
    return this.log;
};
...
CNAPI.prototype.createCnapiModel = function (options, callback) {
    var self = this;

    self.log.info('Initializing model and connecting to APIs');
    ...
    model.connect(function (error) {
        ...
        self.model.initializeBuckets(callback);
    });
};
...
CNAPI.prototype.initializeHeartbeater = function (callback) {
    var self = this;
...
CNAPI.prototype.initializeUr = function (options, callback) {
    ...
    self.ur.on('serverUnsetup', self.onServerSysinfo.bind(self));
    callback();
    return;
};
...
function (sysinfo, callback) {
    ...
    serverModel.getRaw(function (getError, server) {
        ...
        if (serverModel.exists) {
            ...
            self.log.info('Server that just came up existed in moray');
            server.sysinfo = sysinfo;
            server.last_boot = lastboot;
            server.current_platform = sysinfo['Live Image'];

            self.log.warn('Server %s existed', uuid);
            self.log.warn({sysinfo: sysinfo });
            ...
            if (!server.setup && !sysinfo.hasOwnProperty('Setup')) {
                ...
                checkServerSetup(function (error, isSetup) {
                    if (error) {
                ...
                self.log.info('New server has \'Setup\' sysinfo'
                   + ' property, set to \'%s\'', sysinfo['Setup']);
        ...
        function checkServerSetup(cb) {
            ...
            serverModel.invokeUrScript(
                ...
                function (error, stdout, stderr) {
                    ...
                    var numPools = parseInt(stdout.trim(), 10);
                    self.log.warn('Machine had %d zpools', numPools);
                    if (numPools > 0) {
                        self.log.warn(
                            'Marking machine as setup (had %d pools)',
                            numPools);
                        cb(null, true);
                    } else {
                        self.log.warn(
                            'Marking machine as unsetup (had %d pools)',
                            numPools);
                        cb(null, false);
                    }
                });
...
CNAPI.prototype.onServerSysinfo = function (message, routingKey) {
    var self = this;
