...
CNAPI.prototype.createUfdsServerObject = function (opts, callback) {
    ...
    function create() {
        ...
        server.sysinfo = JSON.stringify(serverSysinfo);
        server.datacenter = self.config.datacenter_name;

        server.uuid = serverSysinfo.UUID;
        server.hostname = serverSysinfo.Hostname;
        server.reserved = 'false';
        server.headnode
            = serverSysinfo['Boot Parameters']['headnode'] ? 'true' : 'false';

        server.status = 'running';
        server.defaultconsole = 'vga';
        server.serial = 'ttyb';
        server.serialspeed = '115200';
        server.objectclass = 'server';

        if (heartbeat) {
            var meminfo = heartbeat.meminfo;
            server.memoryavailablebytes = meminfo.availrmem_bytes.toString();
            server.memoryarcbytes = meminfo.arcsize_bytes.toString();
            server.memorytotalbytes = meminfo.total_bytes.toString();
        }
