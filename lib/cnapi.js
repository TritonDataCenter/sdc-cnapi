...
/**
 * Given a sysinfo object, this funciton will check if the server exists in
 * UFDS. Because the sysinfo message is sent only on startup, if the server
 * does exist in UFDS, we will update the last_boot attribute. If the server
 * does not exist, it will be created in UFDS. In either case, the Redis
 * server cache will be updated to reflect that we currently know abotu this
 * server.
 */
...
CNAPI.prototype.onServerStartup = function (message, routingKey) {
    ...
    self.model.serverBeginSysinfoWorkflow(message);
