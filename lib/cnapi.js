...
function (sysinfo, callback) {
    ...
    serverModel.getRaw(function (getError, server) {
        ...
        if (serverModel.exists) {
            ...
            } else if (!server.setup && sysinfo.hasOwnProperty('Setup')) {
            ...
            self.log.info(
                'Server %s not found in moray, creating a record for it',
                uuid);
            checkServerSetup(function (error, isSetup) {
                if (error) {
                    self.log.error(error);
        ...
        function checkServerSetup(cb) {
            ...
            serverModel.invokeUrScript(
                ...
                function (error, stdout, stderr) {
                    ...
                    var numPools = stdout.trim().split('\n').length;
        ...
        function create(opts) {
            serverModel.create(
                { setup: opts.setup, sysinfo: sysinfo, last_boot: lastboot },
                function (error, s) {
                    if (error) {
                        self.log.error(error, 'Error creating server');
                        return;
                    }
