...
RebootPlan.update = function (req, res, next) {
    ...
    function modifyPlan(_, next_) {
        rebootPlan.get(function (error, plan) {
            ...
            // Prevent re-running a plan in the case the plan was canceled on
            // a previous execution, but there are reboots already initiated
            // pending to complete
            if (req.params.action === 'run' ||
                    req.params.action === 'continue') {
                ModelReboot.list({
                    reboot_plan_uuid: plan.uuid
                    ...
                    reboots = reboots.filter(function (r) {
                        return (r.state === 'initiated' ||
                                r.state === 'rebooting');
                    });

                    if (reboots.length) {
                        next_(new restify.InvalidArgumentError(
                                'There are %d reboots pending to finish ' +
                                'from a previous execution of the plan',
                                reboots.length));
                        return;
                    }
