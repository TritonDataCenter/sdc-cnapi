...
RebootPlan.update = function (req, res, next) {
    ...
    assert.optionalBoolean(req.params.single_step, 'single_step');
    var rebootPlan = new ModelRebootPlan(req.params.reboot_plan_uuid);
 ...
 * @param {String} finished_at ISO 8601 formatted Date strin
 * @param {String} canceled_at ISO 8601 formatted Date stringg
 * @response 204 None state of the object has been successfully changed.
 * @response 404 Not Found a reboot plan with the provided UUID
...
RebootPlan.updateReboot = function (req, res, next) {
    ...
    assert.optionalString(req.params.canceled_at, 'canceled_at');
    ...
    if (req.params.canceled_at && isNaN(Date.parse(req.params.canceled_at))) {
        next(new restify.InvalidArgumentError(
                'canceled_at must be a valid ISO 8601 Date string'));
        return;
    }
