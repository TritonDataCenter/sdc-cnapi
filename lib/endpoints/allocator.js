            ...
            if (!server) {
                var httpErr = {
                    ...
                    body: {
                        ...
                        steps: stepSummary,
                        message: 'No allocatable servers found. Last step ' +
                                 'was: ' + stepSummary.slice(-1)[0].step
                    }
                };

                logFailure(req.log, serverDetails, req.params);
...
function getOpenProvisioningTickets(log, cb) {
    ...
    var filter = '&(scope=vm)(action=provision)(status=active)';
...
/*
 * Sometimes ops see allocation failures and disagree with the reasoning that
 * DAPI provided about the failure. When that happens, it's handy to have a
 * snapshot in the logs of what DAPI saw.
 *
 * This is an out-of-band async function, so it never invokes a callback.
 */

function logFailure(log, servers, params) {
    ...
    return zlib.gzip(json, function (err, gz) {
        ...
        log.info({ snapshot: gz.toString('base64') },
                 'Snapshot of request and servers as seen by DAPI');
