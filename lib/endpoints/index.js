...
function attachTo(http, app) {
    ...
    http.get('/diagnostics', function (req, res, next) {
        ...
        if (app.config.serverTaskTransport === 'amqp') {
        ...
        Object.keys(app.waitlistDirector.pendingTicketsByValues).forEach(
            function (k) {
                var t = app.waitlistDirector.pendingTicketsByValues[k];
                ...
                t.tickets.forEach(function (tuuid) {
                    send.waitlist_queues[k].push(
                        ...
                        app.waitlistDirector.pendingTicketsByUuid[tuuid])));
                });
            });

        send.active_waitlist_queues =
            app.waitlistDirector.pendingTicketsByValues;
