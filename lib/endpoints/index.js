...
function ensureConnectedToMoray(opts, req, res, next) {
    opts.log.trace('ensuring connected to Moray');
    next();
}

function ensureConnectedToAMQP(opts, req, res, next) {
    opts.log.trace('ensuring connected to AMQP');
    next();
}

function ensureConnectedToRedis(opts, req, res, next) {
    opts.log.trace('ensuring connected to Redis');
    next();
}

function ensureConnectionTimeout(opts, req, res, next) {
    opts.log.trace('ensuring timeout set');
    req.connection.setTimeout(opts.timeout);
    next();
}

function ensure(opts, callback) {
    var checks = [];
    ...
    if (Array.isArray(opts.connected)) {
        opts.connected.forEach(function (service) {
