...
function ensureConnectedToRedis(opts, req, res, next) {
    if (!opts.model.redis.isConnected()) {
        next(new restify.InternalError(
...
function ping(model, req, res, next) {
    res.send(200, {
        services: {
            redis:
                (model.redis.isConnected()) ? 'online' : 'offline',
            workflow:
                (model.workflow.client &&
                 model.workflow.connected) ? 'online' : 'offline',
            moray:
                (model.moray._morayClient &&
                 model.moray.connected) ? 'online' : 'offline'
        }
    });
    next();
}
