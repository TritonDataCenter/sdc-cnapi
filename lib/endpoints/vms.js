var restify = require('restify');

function VM() {}


VM.load = function (req, res, next) {
    var self = this;
    var serverUuid = req.params.server_uuid;
    var zoneUuid = req.params.uuid;

    if (!this.model.zones[req.params.server_uuid][req.params.uuid]) {
        next(
            new restify.ResourceNotFoundError("No such zone: " + zoneUuid));
        return;
    }

    self.model.loadVm(
        req.params.server_uuid,
        req.params.uuid,
        function (error, vm) {
            if (error) {
                next(new restify.InternalError(error.message));
                return;
            }
            res.send(vm);
            next();
            return;
        });
};

VM.update = function (req, res, next) {
    var self = this;
    self.model.performVmTask('machine_update', req, res, next);
};

VM.start = function (req, res, next) {
    var self = this;
    self.model.performVmTask('machine_boot', req, res, next);
};

VM.stop = function (req, res, next) {
    var self = this;
    self.model.performVmTask('machine_shutdown', req, res, next);
};

VM.reboot = function (req, res, next) {
    var self = this;
    self.model.performVmTask('machine_reboot', req, res, next);
};

VM.create = function (req, res, next) {
    var self = this;
    self.model.performVmTask('machine_create', req, res, next);
};

VM.destroy = function (req, res, next) {
    var self = this;
    self.model.performVmTask('machine_destroy', req, res, next);
};

VM.nop = function (req, res, next) {
    var self = this;
    self.model.performVmTask('nop', req, res, next);
};

function attachTo(http, model) {
    var toModel = {
        model: model
    };

    // Create VM
    http.post(
        { path: '/servers/:server_uuid/vms', name: 'CreateVm' },
        VM.create.bind(toModel));

    // Load VM's properties from the server
    http.get(
        { path: '/servers/:server_uuid/vms/:uuid', name: 'LoadVm' },
        VM.load.bind(toModel));

    // Load VM's properties from the server
    http.post(
        { path: '/servers/:server_uuid/vms/:uuid/update', name: 'UpdateVm' },
        VM.update.bind(toModel));

    // Start VM
    http.post(
        { path: '/servers/:server_uuid/vms/:uuid/start', name: 'StartVm' },
        VM.start.bind(toModel));

    // Stop VM
    http.post(
        { path: '/servers/:server_uuid/vms/:uuid/stop', name: 'StopVm' },
        VM.stop.bind(toModel));

    // Reboot VM
    http.post(
       { path: '/servers/:server_uuid/vms/:uuid/reboot', name: 'RebootVm' },
        VM.reboot.bind(toModel));

    // Delete a VM
    http.del(
        { path: '/servers/:server_uuid/vms/:uuid', name: 'DestroyVm' },
        VM.destroy.bind(toModel));

    // No-op task
    http.get(
        { path: '/servers/:server_uuid/nop', name: 'DoNop' },
        VM.nop.bind(toModel));

}

exports.attachTo = attachTo;
