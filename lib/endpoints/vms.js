...
VM.load = function load(req, res, next) {
    ...
    try {
        validation.ensureParamsValid(req, res, vmValidationRules);
    }
    catch (e) {
        next(e);
        return;
...
VM.create = function create(req, res, next) {
    ...
    var imgapi_url = req.stash.model.config.imgapi.url;
    var image_uuid = req.params.image_uuid;

    req.params.dataset_url_compression
        = req.params.image.files[0].compression;
    req.params.dataset_url = sprintf(
        '%s/images/%s/file', imgapi_url, image_uuid);
