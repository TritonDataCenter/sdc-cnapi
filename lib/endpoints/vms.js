...
VM.create = function create(req, res, next) {
    ...
    function (error) {
        ...
        if (ModelServer.scache[req.params.server_uuid]) {
            req.log.warn(
                'provisioning but ModelServer.scache[req.params.server_uuid] '
                + 'is not set for %s', req.params.server_uuid);

            ModelServer.scache[req.params.server_uuid]
                       .vms[req.params.uuid] =
                {
...
VM.destroy = function destroy(req, res, next) {
    var state =
        ModelServer.scache[req.params.server_uuid].vms[req.params.uuid].state;

    if (state === 'provisioning') {
        delete ModelServer.scache[req.params.server_uuid].vms[
            req.params.uuid];
    }
