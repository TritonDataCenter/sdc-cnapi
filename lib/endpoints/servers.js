...
WorkflowApi.createClient = function (config) {
    WorkflowApi.client = restify.createJsonClient({
        url: config.url,
    ...
    WorkflowApi.log = config.model.log;
};
...
function createProvisionerEventHandler(jobuuid) {
    return function (taskid, event) {
        ...
        WorkflowApi.client.post(
            '/jobs/' + jobuuid + '/info',
            { mesage: event },
            function (error, req, res, obj) {
                if (error) {
                    WorkflowApi.log.error(
                        error, 'Error posting info to jobs endpoint');
                    return;
                }
                WorkflowApi.log.info('Posted info to workflow jobs endpoint');
            });
    };
}
...
function attachTo(http, model) {
    ...
    WorkflowApi.createClient(model.config.wfapi);
