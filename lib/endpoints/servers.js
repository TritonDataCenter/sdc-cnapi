                ...
                ModelServer.getTaskClient().getAgentHandle(
                    ...
                    function (handle) {
                        handle.sendTask(
                            ...
                            { value: req.params.overprovision_ratios },
                            function (taskHandle) {
                                if (!taskHandle) {
            ...
            handle.sendTask(
                ...
                function (taskHandle) {
                    ...
                    taskHandle.on('event', function (eventName, msg) {
                        if (eventName === 'error') {
                            self.log.error(
                                'Error received while fetching tasks: %s',
                                msg.error);
                            error = msg.error;
                        } else if (eventName === 'finish') {
                            if (error) {
 ...
 *
 * @name ServerAttach
 * @endpoint GET /servers/:server_uuid/attach
 * @section Servers
 *
 * @response 204 None Tasks returned successfully
 * @response 500 Error Could not process request
...
Server.attach = function (req, res, next) {
    ...
    try {
        ...
        upsock = WATERSHED.accept(req, resUpgrade.socket, resUpgrade.head);
        remoteaddr = resUpgrade.socket.remoteAddress;
        req.log.info(
            { remoteaddr: remoteaddr }, 'websocket connection created');
    } catch (ex) {
        req.log.error({ err: ex }, 'error accepting websocket');

        next(new InternalError(ex));
        return;
    }

    upsock.on('text', function (data) {
        try {
            ...
            }, 'error parsing websocket message from');
            return;
        }

        switch (msg.type) {
            case 'register':
                req.log.info('registering %s (%s)',
                    req.params.server_uuid, remoteaddr);
                req.stash.server.register();
                break;
    ...
    upsock.on('end', function (data) {
        req.log.info({ remoteaddr: remoteaddr }, 'websocket connection ended');
        req.stash.server.unregister();
    });
};



Server.updateStatus = function (req, res, next) {
...
function attachTo(http, app) {
    ...
    http.get({
        path: '/servers/:server_uuid/attach',
        name: 'ServerAttach' },
        ensure({
            connectionTimeoutSeconds: 60 * 60,
            app: app,
    ...
    http.post({
        path: '/servers/:server_uuid/status',
        name: 'ServerStatusReport' },
        ensure({
            connectionTimeoutSeconds: 60 * 60,
            app: app,
            prepopulate: [],
    ...
    // Return internal server cache
    http.get('/cache/servers',
        function (req, res, next) {
            res.send(ModelServer.scache);
            next();
        });
