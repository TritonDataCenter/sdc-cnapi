 ...
 * @param {String} encryption Encrypt or not the server zpool. (Boolean string
 *      type) See `man mkzpool -e` option
...
Server.setup = function handlerServerSetup(req, res, next) {
    var rules = {
        ...
        'encryption': ['optional', 'isBooleanString']
    };
    ...
    if (typeof (req.params.encryption) !== 'undefined') {
        params.encryption = (req.params.encryption === 'true');
    }
 ...
 * (Requires cn-agent v2.13.0 or later - TBD.)
 *
    ...
    vasync.pipeline({arg: {}, funcs: [
        ...
        function bailIfNotGivenRecoveryRequirements(ctx, cb) {
            ...
            if (zRec &&
                ctx.params.action === 'activate' &&
                (!zRec.staged ||
                zRec.staged !== ctx.params.recovery_config)) {
                cb(new errors.PreconditionFailedError(
