 ...
 * @param {String} fields Comma seperated string values for which server fields to return with
...
Server.list = function handlerSeverList(req, res, next) {
    ...
    function (error) {
        ...
        // Just return the fields that have been requested.
        var wantedFields = req.query.fields.split(',');
        var resultFields = result.map(function _mapResult(s) {
            var s2 = {};
            wantedFields.forEach(function _forEachField(field) {
 ...
 * @param {String} encryption_enabled Whether the server will setup an
 *      encrypted zpool. (Boolean String type) See `man mkzpool -e` option
 * @param {String} disk_exclude Comma-separated list of disk names to exclude
 *      from the zpool.
...
Server.setup = function handlerServerSetup(req, res, next) {
    var rules = {
        ...
        'disk_exclude': ['optional', 'isStringType']
    };
    ...
    if (typeof (req.params.disk_exclude) !== 'undefined') {
        params.disk_exclude = req.params.disk_exclude;
    }
...
Server.recoveryConfig = function handlerServerRecoveryConfig(req, res, next) {
    ...
    vasync.pipeline({arg: {}, funcs: [
        ...
        function bailIfCnAgentTooOld(ctx, cb) {
            ...
            self.log.debug(
                'server %s cn-agent supports "recovery_config": v%s',
                req.stash.server.uuid, ctx.cnAgent.version);
            cb();
        },
