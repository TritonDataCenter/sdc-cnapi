...
/* BEGIN JSSTYLED */
/**
 * Returns a list of compute nodes present in datacenter.
 *
 * @name ServerList
    ...
    async.waterfall([
        function (cb) {
            ...
            if (req.params.extras) {
                ...
                options.extras = JSON.parse(JSON.stringify(extras));
                ...
                if (extras.capacity) {
                    if (!extras.vms) {
                        keysToRemove.push('vms');
                    }
                    if (!extras.sysinfo) {
                        keysToRemove.push('sysinfo');
                    }

                    options.extras.all = true;
                    options.extras.vms = true;
                    options.extras.sysinfo = true;
                } else if (extras.all) {
                    if (!extras.vms) {
                        keysToRemove.push('vms');
                    }

                    // force vms to be returned since it wouldn't be otherwise
                    options.extras.all = true;
                    options.extras.vms = true;
                }
            } else {
            ...
            ModelServer.list(
                ...
                function (error, s) {
                    ...
                    if (extras && (extras.capacity || extras.all)) {
                        ...
                        var i;
                        if (keysToRemove.indexOf('vms') !== -1) {
                            for (i in s) {
                                delete s[i].vms;
                            }
                        }

                        if (keysToRemove.indexOf('sysinfo') !== -1) {
                            for (i in s) {
                                delete s[i].sysinfo;
                            }
                        }
...
Server.get = function (req, res, next) {
    ...
    var serverObj = new ModelServer(req.params.server_uuid);
    ...
    serverObj.getFinal(
        ...
        function (error, server) {
            req.log.warn({ server: server }, 'Running capacity');
            req.log.debug('Running capacity');
            ...
            req.log.debug('Done running capacity');
            req.log.warn({ server: server }, 'Done running capacity');
