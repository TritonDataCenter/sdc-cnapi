...
Server.reboot = function (req, res, next) {
    async.waterfall([
        function getServerPendingReboot(cb) {
            ModelReboot.pending({
                server_uuid: req.params.server_uuid
            }, function (err, reboot) {
                if (err) {
                    cb(new restify.InternalError(err.message));
                    return;
                }
                cb(null, reboot);
            });
        },
        function rejectIfNoRebootAndPlanGiven(reboot, cb) {
            if (!reboot && req.params.reboot_plan) {
                cb(new restify.InvalidArgumentError(sprintf(
                    'Cannot find a pending reboot for reboot' +
                    ' plan %s and server %s',
                    req.params.reboot_plan,
                    req.params.server_uuid)));
                return;
            }
            cb(null, reboot);
        },
        function rejectIfPendingRebootAndNoPlanGiven(reboot, cb) {
            if (reboot) {
                if (!req.params.reboot_plan) {
                    cb(new restify.InternalError(sprintf(
                        'There is a pending reboot for server %s as' +
                        ' part of reboot plan %s',
                        req.params.server_uuid,
                        reboot.reboot_plan_uuid)));
                    return;
                }

                if (req.params.reboot_plan !== reboot.reboot_plan_uuid) {
                    cb(new restify.InvalidArgumentError(sprintf(
                        'There is a pending reboot for different reboot' +
                        ' plan %s and server %s',
                        reboot.reboot_plan_uuid,
                        req.params.server_uuid)));
                    return;
        ...
        req.stash.server.getRaw(function (error, rawserver) {
            ...
            if (req.params.reboot_plan) {
                params.reboot_plan_uuid = req.params.reboot_plan;
            }

            if (reboot) {
                req.log.info({reboot: reboot}, 'Request pending reboot');
                params.reboot_uuid = reboot.uuid;
            }
