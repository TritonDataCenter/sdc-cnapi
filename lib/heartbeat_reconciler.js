        ...
        function _getStatusObject(ctx, cb) {
            ...
            moray.putObject(cnapiStatusBucket, serverUuid, {
                ...
                if (VError.hasCauseWithName(err, 'EtagConflictError')) {
                    ...
                    // error), we'll record it but not do anything else. Since
                    // the state won't have changed unless we got a fresh
                    // heartbeat, we'll just try again next time the reconciler
                    // is run.
