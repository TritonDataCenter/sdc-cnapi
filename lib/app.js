...
function (sysinfo, refreshcb) {
    ...
    var sysinfoCarryOverParams = [
        'Setup', 'Zpool', 'Zpool Disks', 'Zpool Profile',
        'Zpool Size in GiB'];
    ...
    function (error) {
        self.log.info('Finished processing server (%s) sysinfo', uuid);
        refreshcb();
    });
            ...
            function (cb) {
                if (shouldInspectServer) {
                    inspectServer(function (error, values) {
                        ...
                        lastboot = (
                            new Date(values.boot_time * 1000)).toISOString();
                        ...
                        server.setup = setup;
                        server.sysinfo['Setup'] = setup.toString();
    ...
    function onServerDoesNotExist(callback) {
        ...
        if (shouldInspectServer) {
            inspectServer(function (error, values) {
                ...
                lastboot = (new Date(values.boot_time * 1000)).toISOString();
                if (values.zpool.split(' ').length) {
    ...
    function onServerDoesNotExist2(callback) {
        self.log.info('Server %s has \'Setup\' sysinfo'
            + ' property, set to \'%s\'', sysinfo['UUID'],
            sysinfo['Setup']);
        if (sysinfo['Setup']) {
        ...
        function (readerror, script) {
            ...
            serverModel.invokeUrScript(
                ...
                function (error, stdout, stderr) {
                    ...
                    var vals;
                    try {
                        vals = JSON.parse(stdout.toString());
                    } catch (e) {
                        cb(new Error(
                    ...
                    var numPools = vals.zpool.split(' ').length;
            ...
            function (error, params) {
                if (opts.inspect) {
                    ...
                    if (opts.inspect.zpool) {
                        ...
                        opts.setup = true;
                        sysinfo['Setup'] =  opts.setup.toString();
                    }
                }
                serverModel.create(
                    {
