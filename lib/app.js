...
App.prototype.setupAmqpClient = function () {
    ...
    var connection = self.amqpConnection
        ...
        var sub = once(function () {
            self.heartbeater.on(
                'heartbeat', self.onHeartbeat.bind(self));
        });
            ...
            self.vmChanges(uuid, heartbeat,
                 function (err, vms, changed) {
                    if (changed) {
                        ...
                        self.log.trace({ server_uuid: uuid },
            ...
            if (server) {
                ...
                self.vmChanges(uuid, heartbeat,
                     function (err, vms, changed) {
                        ...
                        serverModel.modify({ vms: vms }, function (moderr) {
                            if (moderr) {
                                self.log.warn(
                                    { err: moderr },
                                    'failure to update server vms in moray');
                                return;
                            }
                        });
                    });
