...
function App(config) {
    ...
    config.serverStatusTransport = config.serverStatusTransport || 'amqp';
    config.serverTaskTransport = config.serverTaskTransport || 'amqp';
...
App.prototype.setupAmqpClient = function () {
    ...
    if (self.config.serverStatusTransport === 'amqp') {
        sub = once(function () {
            self.heartbeater.on(
                'heartbeat',
                function (heartbeat, routingKey) {
                    var uuid = routingKey.split('.')[1];
                    self.onHeartbeat(uuid, heartbeat);
                });
        });
    }
    ...
    if (ModelServer.getApp().config.serverTaskTransport === 'amqp') {
...
App.prototype.onHeartbeat = function (uuid, heartbeat, callback) {
    var self = this;
    ...
    if (ModelServer.scache &&
        ModelServer.scache[uuid] &&
        ModelServer.scache[uuid].sysinfo)
    {
        ...
        self.log.warn(
            'heartbeat from server that did not sysinfo. ur may be down on %s',
            uuid);
        callback();
    }
};
...
function (sysinfo, refreshcb) {
    ...
    serverModel.cacheSetServerSysinfo(sysinfo);
    var server;
...
function (uuid, heartbeat, callback) {
    ...
    serverModel.cacheCheckServerExists(function (cacheError, exists) {
        if (cacheError) {
            self.log.error(cacheError, 'Error listing servers from cache');
            callback(cacheError);
            return;
        }

        if (exists) {
            ifServerExistsInCache();
            return;
        } else {
            ifServerDoesNotExistInCache();
        }
    });

    function ifServerExistsInCache() {
        serverModel.getRaw(function (getError, server) {
            if (getError) {
                self.log.error(
                    getError, 'Error listing servers');
                callback(getError);
                return;
            }

            // If server has not yet been populated yet, skip this
            // heartbeat
            if (!server) {
                self.log.warn('skipping heartbeat');
                callback();
                return;
            }

            self.vmChanges(uuid, heartbeat,
                 function (err, vms, changed) {
                    if (changed) {
                        self.log.info({ server: uuid, vms: vms },
                            'heartbeat vms changed');

                        serverModel.updateCacheFromHeartbeat(
                            heartbeat,
                            { vms: vms, last_heartbeat: ts },
                            callback);

                        server.vms = vms;
                        server.last_heartbeat = ts;

                        serverModel.modify(server, function (moderr) {
                            if (moderr) {
                                self.log.warn({ err: moderr },
                                    'failure to update server');
                                return;
                            }
                        });
                    } else {
    ...
    function ifServerDoesNotExistInCache() {
        serverModel.getRaw(function (getError, server) {
            if (getError) {
            ...
            /**
             * Check whether vms have changed. If there are vm changes, we will
             * update the server record to moray.
             */
            ...
            if (server) {
                ...
                self.vmChanges(uuid, heartbeat,
                     function (err, vms, changed) {
                        // XXX write 'modified' vms to cache
                        if (changed) {
                            self.log.info({ vms: vms }, 'vms changed');
                            serverModel.updateCacheFromHeartbeat(
                                heartbeat, { vms: vms, last_heartbeat: ts },
                                callback);
                        } else {
                            serverModel.updateCacheFromHeartbeat(
                                heartbeat, { last_heartbeat: ts }, callback);
                        }
...
/**
 * Compare the VMs given in a heartbeat with those cached for a particular
 * server.
 */
App.prototype.vmChanges =
function (serveruuid, heartbeat, callback) {
    var self = this;

    var vms = {};
    var existingVms;
    var heartbeatVms = heartbeat.vms;
    var vmuuid;
    var changed = false;

    if (!ModelServer.scache[serveruuid]) {
        existingVms = {};
    } else {
        existingVms = ModelServer.scache[serveruuid].vms || {};
    }
    ...
    if (!heartbeatVms) {
        self.log.warn({ server: serveruuid }, 'heartbeat vms member empty');
        callback(null, {}, false);
        return;
    }

    for (vmuuid in heartbeatVms) {
        ...
        vms[vmuuid] = heartbeatVms[vmuuid];

        if (existingVms[vmuuid] &&
            existingVms[vmuuid].last_modified !==
            heartbeatVms[vmuuid].last_modified)
        {
            changed = true;
            self.log.trace({ vm_uuid: vmuuid },
    ...
    for (vmuuid in existingVms) {
        // If server vm isn't present in heartbeat and the status is
        // provisioning local, keep the local version (existing).
        if (!heartbeatVms.hasOwnProperty(vmuuid) &&
            existingVms[vmuuid].status === 'provisioning')
        {
            vms[vmuuid] = existingVms[vmuuid];
        }
    }

    callback(null, vms, changed);
};
...
App.prototype.onServerStartup = function (message, routingKey) {
    ...
    var serverModel = new ModelServer(uuid);
    serverModel.cacheSetServerSysinfo(message);
