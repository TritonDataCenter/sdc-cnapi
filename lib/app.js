...
App.prototype.haveSysinfoNicsChanged = function (sysinfo, fullNapiList) {
    ...
    for (n in sysinfoNics) {
        ...
        if (!napiNic) {
            newNic = {
                ...
                // XXX Is this right? This is the value that
                // ModelServer.beginSysinfoWorkflow uses...
    ...
    if (changed) {
        self.log.info('Server %s had NIC changes. '
          + 'Marking for execution of sysinfo workflow', sysinfo['UUID']);
        return true;
    }
    return false;
};
