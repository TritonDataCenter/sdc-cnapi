...
function markRebootAsStarted(job, callback) {
    if (!job.params.reboot_plan_uuid || !job.params.reboot_uuid) {
        callback();
        return;
    }
    var cnapiUrl = job.params.cnapi_url;
    var cnapi = restify.createJsonClient({ url: cnapiUrl});
    var rebootUrl = '/reboot-plans/' + job.params.reboot_plan_uuid +
        '/reboots/' + job.params.reboot_uuid;

    var payload = {
        events: [ {
            type: 'reboot_job_initiated',
            time: new Date(job.started).toISOString()
        }],
        state: 'initiated',
        job_uuid: job.uuid
    };

    cnapi.put(rebootUrl, payload, function (error, req, res) {
        if (error) {
            job.log.info('Error updating reboot record');
            job.log.info(error.stack.toString());
            callback(error);
            return;
        }
...
function markRebootAsFinished(job, callback) {
    if (!job.params.reboot_plan_uuid || !job.params.reboot_uuid) {
        callback();
        return;
    }
...
module.exports = {
    ...
    chain: [
        {
            name: 'cnapi.mark_reboot_as_started',
            timeout: 30,
            retry: 1,
            body: markRebootAsStarted
        },
        {
        ...
        {
            name: 'cnapi.mark_reboot_as_finished',
            timeout: 30,
            retry: 1,
            body: markRebootAsFinished
        }
    ]
};
