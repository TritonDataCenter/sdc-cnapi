...
function validateParams(job, callback) {
    if (!job.params['napi_url']) {
        callback(new Error('Must specify napi_url'));
        return;
    }
...
function getExistingNics(job, cb) {
    ...
    var napi = new sdcClients.NAPI({ url: job.params.napi_url });
    var macs = [];
    var queried = 0;
    ...
    for (var n in sysinfo['Virtual Network Interfaces']) {
...
function getServerNics(job, cb) {
    ...
    napi.getNics(job.params.sysinfo['UUID'], function (err, nics) {
        ...
        for (var n in job.params.existingNics) {
            napiNics[job.params.existingNics[n].mac] = job.params.existingNics[n];
        }

        for (var n in sysinfoNics) {
            ...
            var listEqual = function listEqual(a, b) {
                if (!a && !b) {
                    return true;
                }
            ...
            if (Object.keys(newNic).length !== 0) {
                ...
                updateNics.push(newNic);
            }

            delete napiNics[sysinfoNic['MAC Address']];
        }

        job.params.updateNics = updateNics;
        job.params.addNics = addNics;
