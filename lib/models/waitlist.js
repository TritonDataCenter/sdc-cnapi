...
WaitlistDirector.prototype.onUpdate = function (timestamp, tickets) {
    ...
    tickets.forEach(function (ticket) {
            ...
            timestamp && timestamp.toISOString() >= ticket.expires_at)
        {
            ...
            if (self.pendingTicketsByUuid[ticket.uuid]) {
                cbs = self.pendingTicketsByUuid[ticket.uuid].callbacks;
                ...
                active = self.pendingTicketsByValues[kvkey].tickets;
    ...
    if (len) {
        ...
        Object.keys(self.pendingTicketsByValues).forEach(function (k) {
            ...
            if (self.pendingTicketsByValues[k].tickets.length > 0) {
                self.log.info('there were tickets for %s', k);
                ticketuuid = self.pendingTicketsByValues[k].tickets[0];
        ...
        self.log.info({ active_by_uuid: self.pendingTicketsbyUuid });
    }
};
