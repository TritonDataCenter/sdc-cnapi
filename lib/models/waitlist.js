...
// how long, in milliseconds, between checks to moray for ticket updates
...
WaitlistDirector.prototype.start = function () {
    ...
    function intervalFn() {
        var params = { timestamp: lastCheck };
...
WaitlistDirector.prototype.onUpdate = function (timestamp, tickets) {
    ...
    async.forEach(
        ...
        function (ticket, fecb) {
            self.log.info('expires: %s > %s', timestamp, ticket.expires_at);
            var i;
                ...
                self.callbacks[ticket.uuid])
            {
                ...
                fecb();
                return;
            } else if (timestamp && timestamp.toISOString() >
                       ticket.expires_at)
            {
...
ModelWaitlist.ticketsUpdatedSince = function (params, callback) {
    ...
    if (params.timestamp) {
        ...
        // Give me...
        filter = sprintf(
            // updated_at is later than `ts`
            '|(&(!(status=expired))((updated_at>=%s)(created_at>=%s)))' + // +
            // status is not expired, but the `expires_at` is past ts
    ...
    ModelWaitlist.log.info({ filter: filter, ts: params.timestamp }, 'filter');
    ...
    try {
        ...
        ModelWaitlist.log.warn(
            { error: e.message }, 'Got an exception from moray');
    ...
    function processResults() {
        ...
        ModelWaitlist.log.warn({ tickets: tickets }, 'found tickets');
        callback(null, tickets);
    }
};
...
ModelWaitlist.prototype.createTicket = function (params, callback) {
    ...
    function writeTicket(cb) {
        self.pushTicketUpdateQueue(ticket, serverqueue, etag, cb);
    }
};
