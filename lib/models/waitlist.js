...
WaitlistDirector.prototype.onFirstCheck = function (tickets) {
    var self = this;
    ...
    tickets.forEach(function (ticket) {
        if (ticket.status === 'active') {
            var key = common.orderedKVString({
                scope: ticket.scope,
                id: ticket.id,
                server_uuid: ticket.server_uuid
            });
            self.activeTickets[key] = ticket;
        }
    });
};

WaitlistDirector.prototype.onCheck = function (error, ticket) {
    if (self.activeTickets) {
        self.log.info({ active: self.activeTicekts });
    }
    ...
    if (!tickets || !tickets.length) {
        self.log.info({ tickets: tickets },
                      'no tickets updated since', lastCheck);
                      // No updated tickets need attention
                      return;
    }

    self.log.info({ tickets: tickets },
                  'tickets updated since %s', lastCheck);
    }unction (error, tickets) {
}
...
WaitlistDirector.prototype.start = function () {
    ...
    function onFirstCheck() {
    
    }

    function onTicket(error, tickets) 
};
