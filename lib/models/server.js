...
ModelServer.list = function (params, callback) {
    ...
    // We need to be a little bit careful, since we could get the 'end' event
    // before we finish adding to the list. So in both the onRecord and onEnd
    // events we'll check if we've gotten the end message AND we're not in
    // the process of working on one before returning.
    ...
    function onRecord(server) {
        ...
        var serverModel = new ModelServer(server.value.uuid);
        serverModel.setRaw(server.value);
