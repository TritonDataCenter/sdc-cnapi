...
ModelServer.prototype.get = function (callback) {
    ...
    if (self.value.raw) {
        ...
        server = JSON.parse(JSON.stringify(self.value.raw));
        applyCachedValues();
    } else {
        ...
        ModelServer.getMoray().getObject(
            ...
            function (error, obj) {
                ...
                server = JSON.parse(JSON.stringify(obj.value));
...
ModelServer.prototype.valuesFromSysinfo = function (opts) {
    ...
    if (heartbeat) {
        ...
        server.memory.memory_available_bytes
            = meminfo.availrmem_bytes;
        server.memory.memory_arc_bytes = meminfo.arcsize_bytes;
        server.memory.memory_total_bytes = meminfo.total_bytes;
    }
...
ModelServer.prototype.setValues = function (opts, callback) {
    this.value.final = this.valuesFromSysinfo({
...
ModelServer.prototype.create = function (opts, callback) {
    ...
    function (error) {
        self.asSysinfo.last_updated = (new Date()).toISOString();
        self.asSysinfo.status = 'running';

        self.log.debug(this.value.final, 'Creating server in UFDS');
            ...
            function (createError, createdServer) {
                ...
                self.cacheSetServerStatus(
                    ...
                    function (statusError) {
                        ...
                        callback(statusError, self.values);
                    });
            });
    });
};
