...
ModelServer.prototype.updateStatus = function (heartbeat, callback) {
    ...
    self.cacheSetLastHeartbeat(ts, function () {
        self.modify({ last_heartbeat: ts }, function (err) {
            if (err) {
                callback(new verror.VError('updating server timestamp'));
                return;
            }
            callback();
        });
...
ModelServer.prototype.getRaw = function (callback) {
    ...
    if (self.value) {
        this.log.warn('Reusing raw value for %s', uuid);
        server = clone(self.value);
        callback(null, server);
    } else {
...
ModelServer.prototype.initialServerValues = function (opts) {
    ...
    server.default_console = opts.boot_params.default_console || 'serial';
...
function (opts) {
    ...
    async.waterfall([
        ...
        function (wfcb) {
            ...
            var rOpts = {
                path: '/tasks',
                headers: { 'x-request-id': origreq.getId() }
            };
