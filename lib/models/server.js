...
/**
 * Update in memory and in moray the last time we got server identifying
 * itself.
 */

ModelServer.prototype.updateStatus = function (heartbeat, callback) {
    var self = this;
    self.updateFromHeartbeat(heartbeat, callback);
};


ModelServer.prototype.checkTimeout = function (heartbeat, callback) {
    var self = this;
    self.updateFromHeartbeat(heartbeat, callback);
};
...
ModelServer.prototype.updateFromHeartbeat =
function (heartbeat, callback) {
    ...
    function updateMemory(cb) {
        if (!serverobj.sysinfo) {
            self.log.info('skipping memory because !sysinfo');
            cb();
            return;
        }
        var memoryKeys = [
...
function (heartbeat, serverobj) {
    ...
    for (vmuuid in serverobj.vms) {
        ...
        if (!heartbeat.vms.hasOwnProperty(vmuuid) &&
            serverobj.vms[vmuuid].status === 'provisioning')
        {
...
function (self, eventCallback, callback, synccb) {
...
ModelServer.prototype.sendAmqpTaskRequest = function () {
    var self = this;

    var opts, task, params, evcb, cb, req_id, synccb;
    ...
    ModelServer.getTaskClient().getAgentHandle(
        ...
        function (handle) {
            handle.sendTask(
                task,
                params,
                ModelServer.createTaskHandler(self, evcb, cb, synccb));
        });
};
...
function (opts) {
    ...
    var task, params, callback, origreq;
    var synccb;
    var serverAdminIp;
    var client;
