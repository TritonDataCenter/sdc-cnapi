...
ModelServer.init = function (app) {
    ...
    ModelServer.cache = new Datastore({ inMemoryOnly: true });

    ModelServer.cache.ensureIndex({ fieldnName: 'uuid', unique: true });
};
...
ModelServer.prototype.store = function (server, callback) {
    ...
    ModelServer.cache.insert(server, function () {});
...
/**
 * Create a Redis key for the server disk usage cache.
 */
ModelServer.prototype.cacheKeyServerDiskUsage = function () {
    return sprintf('cnapi:servers:%s:disk', this.uuid);
};
...
/**
 * Return a Server's cached disk usage information.
 */

ModelServer.prototype.cacheGetDiskUsage = function (callback) {
    var key = this.cacheKeyServerDiskUsage();
    ModelServer.getRedis()
        .hgetall(key, function (error, values) {
            if (error) {
                callback(error, values);
 ...
 */
ModelServer.prototype.cacheSetServerStatusOLD =
function () {
    var self = this;
...
// XXX remove usage of expire value
...
function () {
    ...
    ModelServer.cache.update(
        { uuid: self.uuid }, { status: status }, { upsert: true }, callback);
};
 ...
 */
ModelServer.prototype.cacheGetServerStatusOLD =
function (callback) {
    var key = this.cacheKeyServerStatus();
...
function (disk, callback) {
    var key = this.cacheKeyServerDiskUsage();
    ModelServer.getRedis().hmset(key, disk, callback);
};
